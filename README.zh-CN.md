...
Vibe Crunch 把 AI 编程时长时间坐在电脑前的工作流，切成一段段 **约 30 秒–3 分钟的微训练 / 活动 / 正念间歇**。当你在 Codex / Claude Code 提交任务后，AI 会立即正常开始工作；Vibe Crunch 独立判断当前是否适合提醒，并在满足条件时随机给出一个短动作或冥想练习。
...
- **今天休息**会停止当天后续自动提醒
- 并发会话共享 1 个待处理提醒
- 弹窗连续 15 分钟无人操作时自动收起，记为独立的“无人操作超时”，不算完成或跳过，也不写 Apple 健康
- 超时后第一次新的任务提交只用于确认用户已经回来并重新开始冷却，不会立刻再弹窗；手动 `now` 不受此限制
- 90 分钟 pending 过期仍保留为 UI 进程异常退出时的兜底清理
...
Vibe Crunch 可以把点击 **完成了** 的微训练或冥想转成 iPhone HealthKit Workout。此功能默认关闭，需要一次性配置 Apple 的“快捷指令”和五个跨设备 Focus。冥想使用 Apple 的 `Mind and Body` Workout 类型，与力量、核心、步行和柔韧度分开记录。
...
详细的一次性配置与验收步骤见 [`docs/apple-health-sync.md`](docs/apple-health-sync.md)。现有四类动作链路已经实机验证；加入冥想后，需要再把 `Vibe Sync Mindfulness` 的打开触发器和 `Mind and Body` 分支手动加到同一个 `Vibe Crunch → Health`。
...
倒计时不会显示在弹窗中。连续 15 分钟没有操作时窗口自动关闭；点击 **换一个** 属于有效操作，因此新动作重新开始 15 分钟。超时不会冒充用户主动跳过，也不会增加每日完成目标。
...
3. `~/.local/bin/vibe-crunch now` 能弹出带四个按钮的中文活动窗口。
...
9. 默认配置显示 30 分钟冷却、15 分钟弹窗无人操作超时、每日完成目标 5 次。
10. 整个默认 Vibe Crunch 流程不会请求摄像头权限。
11. 弹窗连续 15 分钟无人操作后自动关闭；完成、跳过、休息和 Health 事件数都不增加，下一次任务提交不会立即再弹窗。
12. 随机池可以抽到 **正念冥想**；点击完成后，本地统计按 `meditation` 记录。
13. 如果启用了 Apple 健康同步，只有 **完成了** 会发送 Health 事件，超时 / 跳过 / 换一个 / 今天休息不会写入 Workout；冥想映射为 `Mind and Body` 与 `Vibe Sync Mindfulness`。
...
调度器负责冷却时间、按完成次数计算的每日目标、单一 pending、完全随机动作选择、当天休息、无人操作超时和重复 Hook 去重。换动作只修改当前 pending，不创建新的提醒，因此不会扭曲冷却时间和当天调度计数；它属于真实交互，会重新开始隐藏的 15 分钟无人操作计时。
