...
def load_config() -> dict:
    ...
    legacy_default_pool = [name for name in MICRO_EXERCISES if name != "meditation"]
    if cfg.get("exercise_pool") == legacy_default_pool:
        cfg["exercise_pool"] = list(MICRO_EXERCISES)
...
def _record_offer(offer: dict) -> None:
    def _record(stats):
        ...
        ex = stats.setdefault("by_exercise", {}).setdefault(
            offer["exercise"],
            {"offered": 0, "completed": 0, "skipped": 0, "rested": 0, "timed_out": 0},
        )
...
def resolve_offer(offer_id: str, action: str, feedback=None):
    ...
    def _apply(state):
        return apply_action(state, offer_id, action, now=completed_ts, feedback=feedback)
...
def _dialog_message(offer: dict) -> str:
    ...
    return (
        f"{actor} 正在卷代码，你先给身体和注意力一个短暂间歇吧。\n\n"
        f"本轮：{offer['label']}\n"
        f"目标：{offer['sets']} 组，{offer['target']}\n\n"
        f"活动要点：{offer['cue']}\n\n"
        "预计约 30 秒–3 分钟，以舒适、可控为准。\n\n"
        "按钮说明：\n"
        "• 完成了：记录本次活动完成\n"
        "• 换一个：随机换成另一个活动，不算跳过\n"
...
def _tk_dialog(offer: dict, timeout_s: float) -> Optional[str]:
    if timeout_s <= 0:
        return "timeout"
    try:
    ...
    result = {"action": "timeout", "chosen": False}
    try:
        root = tk.Tk()
        root.title("Vibe Crunch｜活动间歇")
        ...
        root.lift()
        root.focus_force()
        root.after(max(1, round(timeout_s * 1000)), lambda: choose("timeout"))
        root.mainloop()
    except Exception:
        return None
...
def prompt_offer(offer_id: str, timeout_s: float | None = None) -> int:
    if timeout_s is None:
        timeout_min = max(1, int(load_config().get("dialog_timeout_min", 15)))
    ...
    while True:
        ...
        if not offer:
            return 0
        action = _tk_dialog(offer, timeout_s)
        if action is None:
            action = _mac_dialog(offer, timeout_s) if sys.platform == "darwin" else "skip"
        if action == "swap":
        ...
        if action == "done" and offer.get("program") == "lean" and offer.get("kind") == "strength":
            ...
            feedback = feedback_dialog(timeout_s)
            if feedback == "timeout":
                resolve_offer(offer_id, "timeout")
                return 0
...
def status_text() -> str:
    ...
    lines = [
        ...
        f"弹窗无人操作：{cfg.get('dialog_timeout_min', 15)} 分钟后自动收起",
        ...
        f"    累计无人操作：{stats.get('timed_out', 0)} 次",
    ]
    if pending:
...
def main(argv=None) -> int:
    parser = argparse.ArgumentParser(prog="vibe-crunch", description="Vibe Crunch 活动间歇控制")
